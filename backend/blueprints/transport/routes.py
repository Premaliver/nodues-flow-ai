...
"""Transport department routes — transport fee clearance."""
...
@login_required
@department_access("transport")
def dashboard():
    from utils.tenant_helpers import get_current_context_university
    univ = get_current_context_university()
    return render_template("transport/dashboard.html", university=univ)


@transport_bp.route("/api/dashboard")
@jwt_required()
def dashboard_data():
    ...
    if u_id:
        ...
        transport_dept = Department.query.filter_by(role="transport", university_id=u_id).first()
    ...
    pending = ApplicationDepartment.query.filter_by(
        department_id=transport_dept.id, status="pending"
    ).count()
    approved = ApplicationDepartment.query.filter_by(
        department_id=transport_dept.id, status="approved"
    ).count()
    rejected = ApplicationDepartment.query.filter_by(
        department_id=transport_dept.id, status="rejected"
    ).count()
    ...
    query = (
        ...
        .filter(
            ApplicationDepartment.department_id == transport_dept.id,
...
@transport_bp.route("/api/process/<app_dept_id>", methods=["POST"])
@jwt_required()
...
def process_application(app_dept_id):
    ...
    notification = Notification(
        ...
        title=f"Transport department {action} your application",
        message=remarks,
    ...
    audit = AuditLog(
        ...
        details={"application_id": str(application.id), "department": "transport", "action": action},
