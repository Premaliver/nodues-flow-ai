...
def _send_direct_smtp(server, port, use_tls, username, password, sender_name, sender_email, recipient_email, recipient_name, subject, html_content, otp):
    ...
    try:
        ...
        if not sent:
            raise last_err or RuntimeError(f"Could not connect to {server_host}")
...
def send_otp_email(recipient_email: str, recipient_name: str, otp: str, expires_in_minutes: int = 10, async_send: bool = True) -> dict:
    ...
    try:
        ...
        mail_user = current_app.config.get("MAIL_USERNAME", "smartnodue@gmail.com")
        mail_pass = current_app.config.get("MAIL_PASSWORD", "slidcseebrmtqxng")
        
        # Official university sender branding
        raw_sender = current_app.config.get("MAIL_DEFAULT_SENDER", "Smart NoDues AI <smartnodue@gmail.com>")
        sender_email = "smartnodue@gmail.com"
        sender_name = "Smart NoDues AI"
